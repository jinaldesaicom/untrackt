...
import { aiLearningResources } from '../../data/aiLearning.js'
import { categoryColorMap, levelColorMap, pricingColorMap } from './AILearningCategoryTabs.jsx'
import { aiLearningCategories } from '../../data/aiLearning.js'
...
function AILearningSearch({ onVisit }) {
  ...
  const fuse = useMemo(() => new Fuse(aiLearningResources, fuseOptions), [])
    ...
    <div ref={containerRef} className="relative">
      <div className="relative">
        ...
        <input
          ...
          placeholder='Search courses, tutorials, books... Press "/"'
      ...
      {showResults && results.length > 0 && (
        <div className="absolute z-50 top-full mt-2 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl overflow-hidden max-h-96 overflow-y-auto">
          {results.map((r) => {
            ...
            const catLabel = aiLearningCategories.find((c) => c.id === r.category)?.shortName || r.category
            return (
              <button
                key={r.id}
...
export default memo(AILearningSearch)
