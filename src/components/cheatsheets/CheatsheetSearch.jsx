...
import CheatsheetCardCompact from './CheatsheetCardCompact.jsx'
...
function CheatsheetSearch({ onVisit }) {
    ...
    <div className="w-full">
      <div className="relative">
        ...
        <input
          ...
          placeholder='Search Git, Docker, React...  Press "/"'
      ...
      {results.length > 0 && (
        <div className="mt-3 space-y-1.5">
          ...
          {results.map((sheet) => (
            <CheatsheetCardCompact key={sheet.id} sheet={sheet} onVisit={onVisit} />
          ))}
        </div>
      )}
      ...
      {query.trim().length >= 2 && results.length === 0 && (
        <p className="mt-3 text-sm text-gray-500 dark:text-gray-400 text-center py-4">
          No cheatsheets found for &ldquo;{query}&rdquo;
        </p>
      )}
    </div>
  )
...
export default memo(CheatsheetSearch)
