...
import SDSearch from '../components/systemDesign/SDSearch.jsx'
import SDCategoryTabs from '../components/systemDesign/SDCategoryTabs.jsx'
import SDCard from '../components/systemDesign/SDCard.jsx'
...
const jsonLd = {
  ...
  '@type': 'ItemList',
  name: 'System Design Resources Directory',
  description: `Curated directory of ${getTotalCount()}+ system design learning resources`,
...
export default function SystemDesignDirectory() {
  const [searchParams, setSearchParams] = useSearchParams()
  const activeCategory = searchParams.get('cat') || 'all'
  const { favorites, toggleFavorite, isFavorite } = useSystemDesignFavorites()
  const { recentResources, addRecent } = useRecentSystemDesign()
  ...
  const catLabel =
    activeCategory === 'all'
      ? 'all'
      : systemDesignCategories.find((c) => c.id === activeCategory)?.name || activeCategory
  ...
  return (
    <div>
      <SEOHead
        title={`System Design Resources — ${totalCount}+ Courses, Books, Blogs & More | UnTrackt`}
        description="Curated directory of 100+ system design learning resources. Courses, books, videos, blogs, interview prep, case studies, and tools for mastering distributed systems and scalable architecture."
        path="/system-design"
        jsonLd={jsonLd}
      />
      ...
      <section className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 text-center">
          ...
          <p className="text-lg text-gray-500 dark:text-gray-400 mb-2 max-w-xl mx-auto">
            Curated collection of {totalCount}+ resources to learn system design and distributed systems
          </p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mb-6">
            From fundamentals to FAANG interview prep · Free and paid · All levels
          </p>
          <div className="flex flex-wrap justify-center gap-3 text-xs sm:text-sm">
            ...
            <span className="inline-flex items-center gap-1 rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1 text-indigo-700 dark:border-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-200">
              {systemDesignCategories.length - 1} categories
            </span>
          ...
          <section className="mt-12">
            ...
            <div className="space-y-1.5">
              {recentResources.map((resource) => (
                <SDCardCompact key={resource.id} resource={resource} onVisit={addRecent} />
              ))}
            </div>
          </section>
        )}
