...
const STORAGE_KEY = 'untrackt_dod_checklist'
...
export default function DefinitionOfDoneChecklist() {
  ...
  const copyChecklist = () => {
    const text = `Definition of Done Checklist\n${'='.repeat(30)}\n\n` +
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="text-sm text-gray-500 dark:text-gray-400">{checkedCount}/{items.length} complete</div>
          {allDone && <span className="text-xs font-medium text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-2 py-0.5 rounded-full">✓ Done!</span>}
        </div>
      ...
      <div className="h-2 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-300 ${allDone ? 'bg-green-500' : 'bg-indigo-500'}`} style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="flex items-center gap-2">
        <input type="text" value={newItem} onChange={e => setNewItem(e.target.value)}
          ...
          className="input-field text-sm flex-1" placeholder="Add a new DoD criterion..." />
