...
export function StatCard({ label, value, helper, tone = 'indigo', className = '' }) {
  const toneMap = {
  ...
  return (
    <div className={`rounded-2xl border p-4 ${toneMap[tone] || toneMap.gray} ${className}`.trim()}>
...
export function SegmentedToggle({ options, value, onChange, className = '' }) {
  return (
    <div className={`inline-flex rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden ${className}`.trim()}>
      {options.map((option) => (
        <button
          key={option.value}
          type="button"
          onClick={() => onChange(option.value)}
          className={`px-3 py-2 text-sm font-medium transition-colors ${value === option.value ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'}`}
        >
          {option.label}
        </button>
      ))}
